...
async function loadInvoices() {
  try {
    invoicesData = localStorage.getItem("Delivary_Order_LD") ? JSON.parse(localStorage.getItem("Delivary_Order_LD")) : [];
...
const Archive_Order = JSON.parse(localStorage.getItem("Archive_Order_LD")) || [];
...
const rowToRemove = Delivary_Order_LD.find(r => r[0] == invoiceNumber); // استخدم == بدل ===

// 4️⃣ أضف للطلبات المقبولة
if(rowToRemove){
    Archive_Order.push(rowToRemove);
    localStorage.setItem("Archive_Order_LD", JSON.stringify(Archive_Order));

    // 5️⃣ حذف الصف من Orders_LD
    Delivary_Order_LD = Delivary_Order_LD.filter(row => 
        row.map(String).join(",") !== rowToRemove.map(String).join(",")
    );
    localStorage.setItem("Delivary_Order_LD", JSON.stringify(Delivary_Order_LD));

    showAlert('تم توصيل الطلب  ✅')
    setTimeout(() => {
...
if(rowToRemove){
    ...
    Cook_Order_LD.push(rowToRemove)
